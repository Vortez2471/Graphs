...
void prims()
{
    ...
    for ( auto it : adjlist[1])
        s.insert({it.sec,{1,it.fi}});
    ll ans = 0;
    for ( ll i = 1; i <= n-1; i ++)
    {
        ans += (*s.begin()).fi;
        ll node;
        if ( !mark[(*s.begin()).sec.fi] )
            node = (*s.begin()).sec.fi;
        else
            node = (*s.begin()).sec.sec;
        mark[node]=1;
        for ( auto it : adjlist[node] )
        {
            if ( mark[it.fi] )
                s.erase({it.sec,{min(node,it.fi),max(node,it.fi)}});
            else
                s.insert({it.sec,{min(node,it.fi),max(node,it.fi)}});
        }
    }
    cout << ans;
}
